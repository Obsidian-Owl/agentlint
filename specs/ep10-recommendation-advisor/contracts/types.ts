/**
 * EP10 Recommendation Advisor - Type Contracts
 *
...
export const TracedOriginSchema = z
  ...
  .refine(
    (data) => data.findingId || data.sessionId || data.configGap || data.pattern,
...
export const RecommendationSchema = z.object({
  ...
  status: z.enum(['open', 'pending_confirmation', 'implemented', 'monitoring']),
  events: z.array(RecommendationEventSchema).min(1),
  completedAt: z.string().datetime().optional(),
  completionReason: z.enum(['implemented', 'superseded', 'obsolete', 'rejected']).optional(),
