...
> Context-engineered system prompt for the `generalized-analyzer` subagent.
> This document serves as the template for the prompt string in `src/act/instructions/generalized.ts`.
...
**Orchestrator invokes**: "Analyze the AI tool configuration for this project"

**Your approach**:
1. Call `discover_configs` to scan for all patterns
2. Review discovered files
3. Attempt to classify the tool type
4. If specific tool detected, note that a specialist analyzer exists
5. Apply heuristic quality assessment
6. Return structured findings with clear limitations

**Example finding**:
```
## Analysis Summary

**Detected ACT Type**: agents-md
**Confidence**: medium

## Detection Results

Found AGENTS.md at repository root. No tool-specific configuration
detected (.claude/, .cursor/, .aiderrules not present).

This appears to be a tool-agnostic AGENTS.md configuration.

## Limitations

- Cannot determine which AI tool actually uses this file
- Cannot assess tool-specific best practices
- Session/history analysis not available for generic AGENTS.md
```
