/**
 * EP11 Quality & Security - Secret Detection Contracts
 *
 * TypeScript interfaces for secret detection and classification.
 *
 * @module contracts/secrets
 */
...
/**
 * Redaction format: [REDACTED:type:len=N]
 */
 ...
 */
export function createRedactedPlaceholder(ruleId: string, length: number): string {
  return `[REDACTED:${ruleId}:len=${length}]`;
}

/**
 * Parse a redacted placeholder.
 */
export function parseRedactedPlaceholder(placeholder: string): RedactedValue | null {
  const match = placeholder.match(/\[REDACTED:([^:]+):len=(\d+)\]/);
...
/**
 * Entropy thresholds for classification hints.
 */
export const ENTROPY_THRESHOLDS = {
  /** Below this: likely a placeholder or template */
  LOW: 3.0,

  /** Above this: likely a real secret */
  HIGH: 4.0,
} as const;
