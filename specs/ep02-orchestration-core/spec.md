...
Exposing layer awareness to the agent would constrain its reasoning. The Two-Layer Architecture is an *implementation detail* for tool authors (EP05-EP10), not an orchestration concept. The agent shouldn't know or care whether a tool is "static" or "agentic"—it invokes tools based on task requirements.
...
- NFR-002 adjusted: Context management targets the *configured model's* context window (dynamically queried or hardcoded per model)
