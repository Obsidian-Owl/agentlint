...
- **Progressive Value principle**: Must work without complex setup
...
| VII. Static-First | Yes | Change detection is static; LLM for synthesis only |
| VIII. Progressive Value | Yes | Works without LLM for change detection + staleness |
...
agentlint config frequency           # Show current frequency mode
agentlint config frequency calm      # Switch to calm mode (~$1-5/month)
agentlint config frequency regular   # Switch to regular mode (~$10-20/month)
agentlint config frequency active    # Switch to active mode (~$30-50/month)

# Digest (calm mode)
agentlint digest                     # Show current digest summary
agentlint digest --send              # Force send digest now

# Hooks
agentlint hooks install              # Install hooks for current frequency mode
agentlint hooks uninstall            # Remove all agentlint hooks

# Recommendation lifecycle
agentlint recommend                  # Show current recommendations
agentlint recommend --mark-adopted 123  # Confirm adoption
agentlint recommend --history        # Show recommendation lifecycle history

# Status (non-intrusive staleness check)
agentlint status                     # Show staleness, pending recommendations
agentlint status --verbose           # Detailed change manifest

# Shell prompt (opt-in only)
agentlint shell-init                 # Output shell integration script
# Usage: eval "$(agentlint shell-init)" >> ~/.zshrc
```
