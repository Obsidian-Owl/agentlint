...
export async function initializeDatabases(
  ...
  try {
    ...
    result.warnings.push(
      `Failed to initialize baselines database: ${error instanceof Error ? error.message : String(error)}`
    );
  }
  ...
  try {
    ...
    result.warnings.push(
      `Failed to initialize learnings database: ${error instanceof Error ? error.message : String(error)}`
    );
  }
  ...
  try {
    ...
    result.warnings.push(
      `Failed to create global directory ${globalDir}: ${error instanceof Error ? error.message : String(error)}`
    );
  }
  ...
  try {
    ...
    result.warnings.push(
      `Failed to initialize sessions database: ${error instanceof Error ? error.message : String(error)}`
    );
  }
  ...
  if (includeGlobal) {
    ...
    try {
      ...
      result.warnings.push(
        `Failed to create global learnings directory: ${error instanceof Error ? error.message : String(error)}`
      );
    }
    ...
    try {
      ...
      result.warnings.push(
        `Failed to initialize global learnings database: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
