 ...
 *
 * Specialized subagent for understanding Claude Code sessions.
 * Uses EP08 ACT pattern per ADR-0005.
 *
...
/**
 * Session Analyst prompt.
 * Follows the 4-layer structure: Role → Domain → Task → Output
 *
 * Per ADR-0019: Tools provide data, agent provides judgment.
 * This prompt focuses on interpretation and narrative synthesis.
 *
 * Prompt size: ~7KB (well under NFR-002 50KB limit)
 */
const SESSION_ANALYST_PROMPT = `## ROLE IDENTITY
...
/**
 * Session Analyst subagent instructions.
 *
 * Specialized for understanding Claude Code sessions.
 */
export const sessionAnalystInstructions: SessionAnalystInstructions = {
  ...
  prompt: SESSION_ANALYST_PROMPT,
  tools: [...SESSION_ANALYST_TOOLS],
  priority: 80, // High priority for session analysis
};
