...
export function computeConfidenceLevel(
  factors: Omit<ConfidenceScore, 'overall'>
): ConfidenceLevel {
  const score = [
    factors.specificity,
    factors.temporal,
...
export const TraceIssueInputSchema = z.object({
  ...
  issueLocation: PositionSchema.optional().describe(
    'File and line where issue was found'
  ),
...
export const GetPatternsInputSchema = z.object({
  ...
  minFrequency: z
    ...
    .min(1)
    .optional()
    .describe('Minimum pattern frequency'),
