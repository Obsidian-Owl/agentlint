...
async function forwardToHoneyHive(events: TelemetryEvent[], requestId: string): Promise<void> {
  ...
  for (const [sessionId, sessionEvents] of eventsBySession) {
    try {
      ...
      // T035: Map agent identity to session config
      if (sessionStartEvent?.data?.['gen_ai.agent.id']) {
        sessionConfig['gen_ai.agent.id'] = sessionStartEvent.data['gen_ai.agent.id'];
      }
      if (sessionStartEvent?.data?.['gen_ai.agent.name']) {
        sessionConfig['gen_ai.agent.name'] = sessionStartEvent.data['gen_ai.agent.name'];
      }
